...
I'm only pulling years 2014 through 2020 since the dependent variable (Hospital Volume, YoY%) is only observed in this time range.
...
We have too many records. There should be at most 7*12 = 84 records per `seriesID`, but we actually have...

```{r}
## the first 10 codes
table(dat.bls$seriesID)[1:10]
```

Looking closer at one example...

```{r}
dat.bls %>% filter(seriesID == "CES6562000134",
                   year == 2014,
                   period == "M01")
```

There appear to be a varying number of replicates for each monthly measure of each seriesID. Not sure the reason - maybe something to do with more specific class codes grouped within a seriesID in the BLS database and the API is returning one record for each. Since the reported values are the same, I'll resolve the issue by selecting the first record for each.
...
We can use `auto.arima()` to apply these heuristics to determin the correct specification for an ARIMA model.
...
We only have 81 observations of $y$, so we will remove these features with 35+ `NA`. For those with fewer (5 or less) `NA`, we will use use Last Observation Carried Forward (LOCF) to handle most `NA`. This avoids introducing forward-looking info. For any `NA` appearing at the very start of a series, we impute the mean for the series so as to keep the observations for analysis.
...
lagged_x <- x.ts %>%
  ...
  slice(-c(1:3)) %>%
  ts(start = c(2014, 4), frequency = 1) ## max lag is 3, so drop first 3 records for simplicity
```
...
```{r}
m4 <- gam(model_form,
          data = tibble(y = y.ts_month) %>% cbind(., X_pca %>% as_tibble) %>% slice(1:69),
          family = gaussian(),
          method = "GCV.Cp",
          select = TRUE)
