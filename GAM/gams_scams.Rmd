...
Our leading model so far seems to be the GLM with a log-transform. Is this the best we can do?
...
A GAM is fit using `mgcv`'s `gam()` and `s()` methods as follows
...
We notice that we spend about 5 degrees of freedom estimating the smooth term `s(LicAge)`.
...
$k$ represents the basis dimension for the smooth. What is the dimension of our selected spline basis? Well since we know we should be estimating 1 $\beta$ for each basis function, let's check
...
Let's now confirm our understanding by getting predictions a few different ways, we should get the same result from each.
...
Here we can see that the "pressure" of the signal in our data is only strong enough to bend the plate at the corner where both `DrivAge` and `LicAge` are low, and elsewhere we have essentially a flat plate, i.e. a plane. We can infer from this that we did not shrink our the linear parts of the smooths, only the wiggly bits. That is the default behavior - shrinkage is applied in the "range space" containing the non-linear basis functions, not to the "null space" with the linear components of each term. But there are a couple ways to also apply shrinkage here if we like.
...
The double penalty approach produces more regularlization on the linear components than the shrinkage splines. These abilities become powerful when you have more than a handful of features and want to perform automatic feature selection, as in a Lasso regression.
...
We've expressed our desire for a non-wavy curve as a shape constraint within the spline basis itself, and estimated the optimal curve given the data under that condition. This is also possible in the bivariate case by using the argument `bs="tedmd"`, though I will not do so here because it generates warnings due to not being an appropriate model for this data.

Now let's consider the predictive performance of our scam model as compared to our first GAM with no shape constraint.

```{r}
AIC(scam_mono_d, gam_tps_1)
```

We end up with a less complex model, with the desired shape, and which performs comparably to the original based on the AIC (an estimate of out-of-sample prediction performance). I also think we produce a better result for our business partners which incorporates some of the constraints they have on what can be implemented.
